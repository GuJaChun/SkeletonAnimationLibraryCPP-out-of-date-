...
--[[
	...
	fac = db.Cocos2dxFactory:new();
	fac:loadSkeletonFile("res/skeleton.xml" , "");
	fac:loadTextureAtlasFile("res/texture.xml" , "");
	...
	for i = 1 , 10 do
		...
		local arm = fac:buildArmature("Zombie_polevaulter" , "" , "Zombie");
		table.insert(arms , arm);
		-- 把根节点取出来
		local skeletonNode = tolua.cast(arm:getDisplay() , "db.CocosNode"):getNode();
		-- 设置随机位置
		skeletonNode:setPosition(math.random(100 , 300),math.random(100 , 300));
		-- 添加到场景中
		layer:addChild(skeletonNode);
		
		local function onComplete(name , event)
			print(i , "onComplete" , name , event:getType());
		end
		arm:addListener("complete" , onComplete);

		local function onLoopComplete(name , event)
			print("onLoopComplete" , name , event:getType());
		end
		arm:addListener("loopComplete" , onLoopComplete);
		-- 播放走路动画
